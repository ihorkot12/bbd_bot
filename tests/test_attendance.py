...
from app.services.attendance import find_inactive_from_records, _schedule_time_for_weekday
...
class TestScheduleTimeForWeekday:
    def test_extracts_time_for_matching_day(self):
        # Понеділок
        t = _schedule_time_for_weekday("пн,ср,пт 18:00-18:40", 0)
        assert t is not None
        assert t.hour == 18
        assert t.minute == 0

    def test_returns_none_for_non_matching_day(self):
        # Неділя
        t = _schedule_time_for_weekday("пн,ср,пт 18:00-18:40", 6)
        assert t is None

    def test_parses_time_with_dot_separator(self):
        t = _schedule_time_for_weekday("вт,чт 17.30-18.10", 1)
        assert t is not None
        assert t.hour == 17
        assert t.minute == 30
