...
class LeadService:
    ...
    def mark_trial_present(self, lead_id: str) -> Optional[Lead]:
        ...
        if not lead:
            return None
        return self.set_trial_result(lead_id, True)

    def mark_trial_absent(self, lead_id: str) -> Optional[Lead]:
        lead = self._leads.get_by_id(lead_id)
        if not lead:
            return None
        return self.set_trial_result(lead_id, False)

    def set_trial_result(self, lead_id: str, is_present: bool) -> Optional[Lead]:
        lead = self._leads.get_by_id(lead_id)
        if not lead:
            return None
        lead.status = LeadStatus.TRIAL_DONE
        lead.trial_present = is_present
    ...
    def undo_last_trial_decision(self, lead_id: str) -> Optional[Lead]:
        """
        Відкочує рішення після пробного, щоб можна було виправити помилковий клік.
        """
        lead = self._leads.get_by_id(lead_id)
        if not lead:
            return None
        lead.trial_present = None
        lead.status = LeadStatus.TRIAL_SCHEDULED if lead.trial_date else LeadStatus.NEW
        lead.updated_at = datetime.now()
        self._leads.upsert(lead)
        return lead
